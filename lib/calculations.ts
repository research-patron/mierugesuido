...
function buildDiagnosisComment(label: string, rate: number | null, required100: number | null) {
  ...
  if (label === "適正水準") {
    return `最新決算の経費回収率は${rateText}であり、汚水処理費に対する下水道使用料収入が概ね100%の水準です。`;
  }
  if (label === "やや不足" || label === "要注意") {
    return `最新決算の経費回収率は${rateText}であり、下水道使用料収入が汚水処理費を下回っています。費用や有収水量が変わらない仮定では、事業全体の使用料収入の不足解消には約${revisionText}の増加が必要となる単純計算です。`;
  }
  if (label === "判定不可") {
    return "主要な決算値が未取得のため、使用料水準は判定できません。";
  }
  return `最新決算の経費回収率は${rateText}であり、汚水処理費に対する使用料収入の不足割合が大きい状態です。費用や有収水量が変わらない仮定では、事業全体の使用料収入の不足解消には約${revisionText}の増加が必要となる単純計算です。`;
}
