...
const prefecturePageSource = readFileSync(path.join(root, "components/PrefectureComparisonPage.tsx"), "utf8");

describe("map page shell guardrails", () => {
  it("keeps the national page aligned with the home KPI language and year display", () => {
    for (const label of ["公共下水道の収録自治体数", "本サイト収録の最新決算年度", "公共下水道：100%未満の割合", "R5→R6 施行年月日が変わった団体"]) {
    ...
    expect(nationalPageSource).toContain("changedMunicipalityCount");
  ...
  it("derives prefecture KPIs only from the municipalities shown on the page", () => {
    expect(prefecturePageSource).toContain("const targetMunicipalities = selectComparisonRows(source, params, home.overview.latestYear);");
    ...
    expect(prefecturePageSource).toContain('revisions[item.municipalityCode]?.status === "changed"');

    for (const label of ["対象自治体数", "平均経費回収率", "経費回収率100%未満の割合", "R5→R6 施行年月日の変更"]) {
  ...
  it("uses the refined prefecture shell and accurate comparison language", () => {
    ...
    expect(prefecturePageSource).toContain('href={comparisonHref("/map", params)} className="button-secondary"');
