...
総務省/e-Statの「地方公営企業決算状況調査 / 全国 / 調査表 / 年次」を取り込み、自治体・事業別に下水道使用料水準、経費回収率、使用料収入の必要増加率（単純計算）、料金改定に関する公式記載を表示する Next.js アプリです。
...
- 経費回収率、使用料単価、汚水処理原価、100%/80%/150円m³達成に必要な使用料収入の増加率（単純計算）
- ランキング表示
...
- 100%達成に必要な使用料収入の増加率 = 汚水処理費 ÷ 下水道使用料収入 - 1
- 80%達成に必要な使用料収入の増加率 = 0.8 × 汚水処理費 ÷ 下水道使用料収入 - 1
- 150円/m³達成に必要な使用料収入の増加率 = 150 ÷ 使用料単価 - 1

これらは費用・有収水量等を一定とした事業全体の単純計算であり、家庭用20m³月額への換算や自治体の公式な改定率ではありません。R5・R6の変更一覧は、第33表の現行使用料施行年月日が年度間で変わった事業だけを対象とします。前回使用料改定年月日、公式改定率、家庭用・業務用料金等は関連情報として分けて表示します。
...
家庭用20m³月額と経費回収率等の経営指標は別の入口で確認できます。トップの地図は本サイト収録の最新決算年度に統一し、異年度を補完しません。公共・特環の地図、自治体比較ランキング、検索は共通の対象を使い、URLに年度・事業・会計条件を保持します。

全事業ランキングは既存の先頭50件を公開する方式です。`data/static/ranking-coverage.json`に、同じ集計から得た全母数と入力SHA-256を保持し、別版の母数を誤表示しないようビルド時に検査します。出典の公表日・取得記録は`data/static/source-provenance.json`で、カタログ年・決算年度とは分けて管理します。両ファイルは既存の`static:data`処理でも生成します。

検索・自治体詳細・改定情報の標準表示は初期HTMLに含めます。事業・表示タブ等のクエリはJavaScript実行後に反映され、従来URLも利用できます。無料の都道府県CSVは従来の全区分・代表事業ファイルを維持し、画面の絞り込みとは範囲が異なることを明示します。

調査根拠、変更範囲、検証結果、制約と後続の商品設定は [第1段階の報告](docs/comparison-phase1.md) を参照してください。note販売・計測・公開は今回の実装に含めていません。


## データ・資料セットと履歴の修正

年度付きの詳細URLでも、指定年度までの5年間の履歴を保持します。資料案内は `/datasets/`、最初の商品は `preview` 状態です。価格・販売先・提供条件が未確定のため購入リンクは表示しません。

`pnpm datasets:generate` で同じ比較処理から無料見本とGit管理外の販売用一式を生成し、`python3 scripts/datasets/verify.py` でCSV・ZIP・版の一致を検証します。通常の静的ビルドでは公開見本だけを更新します。計測は初期状態で無効、外部タグ・送信先は追加していません。

設定・更新・訂正・公開前の手順と検証範囲は [第2・3段階の報告](docs/dataset-phase2.md) を参照してください。


### 2020年度の無料CSVと要望アンケート

全国の公共下水道1,189事業会計・174列を無料配布用に生成。原資料の再生成は `pnpm datasets:free2020`、通常ビルドの整合確認は `pnpm datasets:free2020:check`。承認済みGoogleフォームで回答後、CSV・計算用原値・計算式・出典位置を取得できる。回答と集計は運営者限定のGoogleフォーム・回答シートに保存し、サイトへ同期しない。収録範囲、生成・検証、運用と残る公開手順は [実装報告](docs/free-dataset-2020.md) を参照。
