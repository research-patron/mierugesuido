...
import { detailDisclaimer, formulaCopy, siteName } from "@/lib/copy";
import { getMunicipalityDetail, getPrefecturePeerComparison } from "@/lib/data";
...
type MunicipalityDetail = NonNullable<Awaited<ReturnType<typeof getMunicipalityDetail>>>;
...
export default async function MunicipalityDetailPage({
  ...
  params: Promise<{ municipalityCode: string }>;
  searchParams: Promise<{ business?: string | string[]; view?: string | string[] }>;
}) {
  const [{ municipalityCode }, query] = await Promise.all([params, searchParams]);
  const municipality = await getMunicipalityDetail(municipalityCode);
  if (!municipality) notFound();

  const groups = buildBusinessGroups(municipality.businesses);
  const requestedBusiness = firstParam(query.business);
  const view = parseDetailView(firstParam(query.view));
  if (requestedBusiness && !groups.some((group) => group.key === requestedBusiness) && groups[0]) {
    redirect(detailHref(municipalityCode, groups[0].key, view));
  }
  ...
  const trace = parseJson(latest.sourceTraceJson);
  const evidenceEntries = Object.entries(trace)
    .filter(([, item]: [string, any]) => item?.value != null)
    .sort(([a], [b]) => evidenceOrder(a) - evidenceOrder(b))
    .slice(0, 14);
  const financialAnnual = findAnnual(selectedGroup, 2024, latestBusiness.accountingType);
  const previousFinancialAnnual = findAnnual(selectedGroup, 2023, latestBusiness.accountingType);
  const financialStory = buildFinancialStoryModel(financialAnnual ?? {
    sewerBusinessId: latestBusiness.id,
  ...
  const financialStatementsReady = Boolean(financialAnnual?.financialStatementItems.length);
  ...
  const prefecturePeerComparison = view === "prefecture"
    ? await getPrefecturePeerComparison({
        prefectureName: municipality.prefectureName,
        businessKey: selectedGroup.key,
          ...
          <article className={styles.revisionCard}>
            ...
            {municipality.revisionEvents.length > 0 ? (
              <div className={styles.revisionList}>
                {municipality.revisionEvents.map((event) => (
...
function JointOperationLinks({ municipality }: { municipality: MunicipalityDetail }) {
  const memberships = [...new Map(municipality.servedServiceMemberships.map((membership) => [
    `${membership.operatorMunicipalityId}:${membership.businessKey}`,
    membership
  ])).values()];
    ...
    <aside className={styles.jointOperationCard} aria-label="組合運営の関連下水道事業">
      ...
      <div className={styles.jointOperationLinks}>
        {memberships.map((membership) => {
          ...
          return (
            <Link key={`${membership.operatorMunicipalityId}:${membership.businessKey}`} href={`/municipalities/${operatorCode}?${query.toString()}`}>
  ...
  return [...grouped.entries()]
    .flatMap(([key, rows]) => {
      const latestPair = rows
        .flatMap((business) => business.annualFinancials.map((annual) => ({ business, annual })))
        .sort((a, b) => {
...
function findAnnual(group: BusinessGroup, year: number, preferredAccountingType?: string | null) {
  return group.businesses
    .flatMap((business) => business.annualFinancials
      .filter((annual) => annual.surveyYear === year)
      .map((annual) => ({ business, annual })))
    .sort((a, b) => {
