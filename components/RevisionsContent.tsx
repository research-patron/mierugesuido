...
export function RevisionsContent({ initialDataset }: { initialDataset: StaticRevisionDataset }) {
  const [query, setQuery] = useState("");
  const searchParams = useMemo(() => new URLSearchParams(query), [query]);
  const [dataset, setDataset] = useState<StaticRevisionDataset | null>(initialDataset);
  const [loadState, setLoadState] = useState<RevisionLoadState>("ready");
