...
## 2026-09-23 — history repair and dataset product preparation

- Reproduced the preview regression: fiscalYear filtered the underlying annual records before five-year charts. Retained complete business histories while selecting the requested current year; missing years stay missing. Verified all published R6 business histories plus real R6/R5/absent-year browser paths.
- Added the requested management heading above the four home indicators. Removed the fixed parent height after screenshot review exposed clipped cards. Seven navigation destinations fit two mobile rows with readable labels and tap targets.
- Added one real preview product with 1,168 rows, 13 columns and a five-row sample, generated by the existing comparison functions. Kept free UI/CSV and raw financial data unchanged. Full ZIP stays in ignored private-products; public assets contain samples only.
- Product price, seller, note URL and terms remain unset. Purchase stays hidden; preview is noindex and excluded from sitemap. Measurement defaults off with no external transport or tags; outbound clicks never become purchases.
- Inspected final 1491×1055 and 390×844 home, history, dataset listing and product/sample screenshots. New static browser QA: 11 passed including keyboard download and table scrolling; existing phase-1 QA: 15 passed. Runtime errors and external requests in the new suite: zero.
- Full tests: 57 files, 461 passed, no skips with official cached workbooks. Typecheck and static build passed; 1,649 routes and 1,648 sitemap URLs verified. Independent CSV/source/sample/ZIP checks passed, including exact regeneration and 119,728-byte ZIP size.
- Gitleaks and targeted private-information checks passed. No staging, commit, push, production/note publication, purchase, advertising or telemetry transmission in this extension.
- Physical devices, OS IME composition, Safari/Firefox, screen readers, Search Console, parent-site policy/contact coverage and note delivery are unverified. Operator configuration and release checks are in docs/dataset-phase2.md.

final result: passed

## 2026-09-23 — 2020 free dataset and anonymous request survey

- Added the 2020 national public-sewer CSV (1,189 accounts, 174 columns), definitions, source correspondence and plotting instructions. Legal/non-legal scope, shared account identity, missing data and 21 cost metrics are explicit; existing 2024 product settings and free comparison routes stay intact.
- Independent OOXML/CSV verification matched 38,048 official values and all cost conversions, item-specific prefecture medians/differences/counts; 4,676 values agree with published R2 histories. 58 test files / 468 tests passed without skips, typecheck and static build passed (1,651 routes / 1,648 sitemap URLs).
- Inspected desktop 1491×1055 and mobile 390×844 local information/download surfaces, with no horizontal overflow. Keyboard-triggered CSV download event confirmed. One browser observation timed out in automatic approval review; the permitted retry succeeded.
- Created the four-question Google Form and linked response spreadsheet as unpublished drafts in the owner's account. Owner-only editing and restricted sheet sharing, no email collection, no sign-in-required one-response limit, and no public response summary were observed. The form accepts no responses until publication.
- Public end-to-end submission/download and anonymous access checks require approval for form publication, one identified test response, and branch deployment. Physical devices and spreadsheet-app visual inspection remain untested. No main/note publication, push, test submission or analytics transmission performed.

final result: pending external publication approval and end-to-end verification


## 2026-09-26 — survey-first entry and reproducible source inputs

- The normal free-data entry now links directly to the Google questionnaire only after publication/privacy verification. Draft mode keeps an explicit preparation state. The completion destination remains a static soft gate; no answer flags or authentication were added.
- Added raw input values (1,189 accounts, 35 columns), official worksheet row locations (11,324 rows), and a calculation/dependency dictionary for all 174 output columns. Nine official Excel links use the pinned source catalog; raw e-Stat survey tables are explicitly distinguished from municipality yearbook PDFs.
- Independent Python/OOXML checks passed for 78,193 source values/row addresses, all ratios, cost shares, conversions, medians, differences, counts and hashes. Full regression: 58 files / 470 tests passed, typecheck and static build passed (1,651 routes / 1,648 sitemap URLs).
- Inspected desktop 1491×1055 and mobile 390×844 download views and the mobile survey-first information view. No horizontal overflow. The raw-input CSV download event was confirmed using Enter. Original Excel download availability, physical devices, spreadsheet-app rendering, anonymous permissions and the published submission-to-download journey remain unverified.
- Google Form remains unpublished; current owner UI reconnects successfully and shows the four saved questions. No form publication, response submission, staging, commit, push, main or note operation in this follow-up.

final result: pending external publication approval and end-to-end verification


## 2026-09-26 — official workbook reachability and historical layout verification

- Downloaded all nine official workbooks from the published e-Stat links: every SHA-256 matches the source edition used for generation. The official Table 20 download also triggered successfully through the browser.
- Downloaded the R2 survey layouts and instructions, confirmed the 2020 settlement label and all adopted field positions, including drawing-based labels. Inspected rendered instruction pages 202/204 for non-legal tax treatment; the official raw amounts remain unchanged. Added tax basis and historical layout links to sources.csv and corresponding calculation/readme explanations.
- The main CSV is byte-for-byte unchanged. Independent source checks: 78,193 passed; full suite: 58 files / 471 tests passed; typecheck and static build passed.
- Test submission is now authorized by the user. Form publication approval remains pending; the unpublished form cannot accept the authorized response yet. No response was sent and no publication/push occurred.

final result: pending form publication approval and end-to-end verification


## 2026-09-26：無料配布の本番受付設定

運営者がフォーム公開、テスト送信、main反映を承認。Googleフォームで必須未入力を拒否し、識別付きテスト1件だけを保存、所有者の集計に反映。匿名HTTPで回答フォーム200・回答概要403・回答シート401。完了URLを本番へ変更。受付設定をactiveにした。既存の471テスト、静的ビルド、原資料78,193照合に加え、変更後の最終ゲートを実施する。公開後の配信検証はデプロイ完了時に行う。
